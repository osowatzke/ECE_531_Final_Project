...
In this project, we demodulate XM radio transmissions and perform forward error correction (FEC) with our Pluto SDR. XM radio uses a combination of satellites and terrestrial repeaters to diversify its transmitted signal. The satellites use QPSK modulation, and the terrestrial repeaters use coded orthogonal frequency domain multiplexing (COFDM). Our work focuses on the time-division multiplexed (TDM) signals from a single satellite. To demodulate this signal we leverage concepts from the course such as time synchronization, carrier synchronization, and frame synchronization.

However, the focus of th

In this project, we demodulate time-division multiplixed (TDM) XM radio transmissions from a single satellite. XM Radio is a proprietary signal, and most of the details are only documented in patents such as \cite{a2008_us8260192b2, marko_2012_us8667344b2}. Most of the work in this project is developed based on patents.  XM radios use two TDM data streams from satellites as well as a COFDM as well as an or on two satelittes  and % These transmissions  which used QPSK modulation. 

This required concepts learned in class such as time synchronization, carrier synchronization, and frame synchronization. We also attempted to perform forward error correction (FEC) on the received signal.

In this project, we attempted to demodulate time-division multiplexed (TDM) XM radio signals, which were modulated using QPSK modulation. These signals leveraged QPSK modulation and 
